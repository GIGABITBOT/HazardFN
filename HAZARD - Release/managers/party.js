...
module.exports = (app) => {
                ...
                client.functions.SendMessage(JSON.stringify(
                    {
                        ...
                        "account_id": `HazardBot${counter != 0 ? counter : ''}`,
    ...
    app.post('/party/api/v1/*/user/:accountId/pings/:pingerId', (req, res) => {
        ...
        if (req.params.accountId != "HazardBot") return;

        var client = global.xmppClients.find(x => x.accountId == req.params.pingerId);

        if (!client) return;

        client.functions.AddBot("CID_128_Athena_Commando_F_Hazmat")
    })
    ...
    // /party/api/v1/Fortnite/parties/LobbyBotPartyLMFAO/members/HazardBot
    app.delete('/party/api/v1/*/parties/:PartyId/members/:accountId', (req, res) => {
        ...
        if (`${req.params.accountId}`.startsWith("HazardBot")) {
